...
public class WebsocketHandler extends TextWebSocketHandler {
	...
	public void afterConnectionEstablished(WebSocketSession session)
		...
		EmployeeVo pfuserinfo = (EmployeeVo) session.getHandshakeAttributes()
				.get("pfuserinfo");
		...
		if (sessionsMaps.isEmpty()) {
			...
			session.sendMessage(new TextMessage("欢迎："
					+ pfuserinfo.getEmployeename()));
			String name = pfuserinfo.getDepartmentname() + ":"
					+ pfuserinfo.getEmployeename() + "上线了";
			// 给除了他的所有人发上线通知
			sendMessageToUsersByGetOnlies(new TextMessage(name), session);
		} else {
			// 第二个人登录
			Collection<EmployeeVo> isss = sessionsMaps.values();
			// 循环得到value 的employeevo 的id的intvalue的值 与当前
			// pfuserinfo的id的intvalue的值相等时的 value值，根据气质 判断被等这session
			for (EmployeeVo employeeVo : isss) {
				// 判断是否已经登录
				System.out.println("//判断是否已经登录"); // --------------------------------else
				if (employeeVo.getId().intValue() == pfuserinfo.getId()
						.intValue()) {
					...
					for (WebSocketSession webSocketSession : aa) {
						if (sessionsMaps.get(webSocketSession).getId()
								.intValue() == pfuserinfo.getId().intValue()) {
							...
							String name = pfuserinfo.getDepartmentname() + ":"
									+ pfuserinfo.getEmployeename() + "上线了";
							// 给除了他的所有人发上线通知
							sendMessageToUsersByGetOnlies(
									new TextMessage(name), session);
						}
						break;
				...
				// 此账号现在没人登录
					...
					String name = pfuserinfo.getDepartmentname() + ":"
							+ pfuserinfo.getEmployeename() + "上线了";
					// 给除了他的所有人发上线通知
					sendMessageToUsersByGetOnlies(new TextMessage(name),
							session);
	...
	public void afterConnectionClosed(WebSocketSession session,
		...
		String name = pfuserinfo.getDepartmentname() + ":"
				+ pfuserinfo.getEmployeename() + "下线了";
		// 给除了他的所有人发下线通知
		sendMessageToUsersByGetOnlies(new TextMessage(name), session);
	 ...
	 * 上下线提醒
	 */
	public void sendMessageToUsersByGetOnlies(TextMessage message,
	...
	public String makeDown(String sessionId) {
		...
		while (aa.hasMoreElements()) {
			...
			if (session.getId().endsWith(sessionId)) {
				...
				if (session.isOpen()) {
					try {
						String msg = "强制下线通知！用户："
								+ employeeVo.getEmployeename() + "被管理员强制下线了";
						sendMessageToUsersByGetOnlies(new TextMessage(msg),
								session);
